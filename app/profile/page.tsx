...
export default function ProfilePage() {
  ...
  const [tempReadmeContent, setTempReadmeContent] = useState("");
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const { data: readmeData, error: readmeError } = await supabase
          .from("profile_readmes")
          .select("content")
          .eq("user_id", userData.id)
          .single();

        if (!readmeError && readmeData) {
        ...
        if (projectRoles && projectRoles.length > 0) {
          const projectIds = projectRoles.map((role: any) => role.project_id);
          ...
          const projectsWithRoles = joinedProjects?.map((project: any) => {
            const role = projectRoles.find(
              (role: any) => role.project_id === project.id
            );
            return {
              ...project,
              roleTitle: role?.title || "Member",
            };
          });
    ...
    fetchData();
  }, [githubUsername, session, sessionStatus]);

  const handleEditReadme = () => {
    setTempReadmeContent(readmeContent);
  ...
  const handleSaveReadme = async () => {
    try {
      ...
      const { error } = await supabase.from("profile_readmes").upsert({
        user_id: user.id,
        content: tempReadmeContent,
        updated_at: new Date().toISOString(),
      });

      if (error) throw error;

      setReadmeContent(tempReadmeContent);
      ...
      <div className="container mx-auto px-4 py-8">
            ...
            >
              ...
              <div className="flex flex-wrap gap-2">
                {githubData.skills.length > 0 ? (
                  githubData.skills.map((skill: string, index: number) => (
                    <motion.span
                      key={index}
        ...
        >
          <div className="flex justify-between items-center mb-6">
            ...
            {session?.user?.login === githubUsername && (
          ...
          {isEditingReadme ? (
            <div className="space-y-4">
              <textarea
                value={tempReadmeContent}
                onChange={(e) => setTempReadmeContent(e.target.value)}
                className="w-full h-64 p-4 bg-gray-900 border border-gray-700 rounded-lg text-gray-100 font-mono text-sm"
                placeholder="Write your profile README in Markdown..."
              />
              <div className="text-sm text-gray-400">
                <p>Supports GitHub Flavored Markdown (GFM)</p>
                <p>You can use headings, lists, code blocks, etc.</p>
              </div>
            </div>
          ) : (
